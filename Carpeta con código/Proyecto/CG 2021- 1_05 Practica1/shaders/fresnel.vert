...
void main(void)
{
    ...
    vec4 position = in_Position + 0.005*sin(0.02*time) * vec4(in_N, 1.0f) ;
    ...
    vs_fs_normal = mat3(view*model) * in_N;
    ...
    vec3 worldNormal = normalize( mat3( model[0].xyz, model[1].xyz, model[2].xyz ) * -in_N );
